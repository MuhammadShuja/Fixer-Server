@extends('back.master')
@section('title')
    <title>Staff ~ {{ config('app.sys') }}</title>
@endsection
@section('meta')
        ...
        <div class="page-content">
            <div class="page-head">
                ...
                <div class="page-toolbar">
                    <a class="btn-c btn-p" href='/admin/settings/staff/new'>Add user</a>
                </div>
            </div>            
    ...
    <script>
        window.onload = function(){
            $(document).ready(function() {
                ...
                $('#indexTable').on('click', 'tbody td', function(){
                   window.open('{{config('APP_URL')}}/admin/settings/staff/'+$(this).closest('tr').find('td:eq(0)').data('link'));
                });
        ...
        function populateData(key){
            var dt = $('#indexTable').on('error.dt', function (e, settings, techNote, message) {
                    ...
                    "processing": true,
                    "serverSide": true,
                    "ajax": "/api/admin/settings/staff?status="+key,
                    "autoWidth":false,
                    fixedHeader: {
                        header: true,
            ...
            var export_filename = '~ {{ config('app.sys') }}_Staff';
